...
var totalFrames = 40; // the number of images in the sequence of JPEG files (this could be calculated server-side by scanning the frames folder)
...
(function animloop(){ // the smoothest animation loop possible
  ...
  targetStep = Math.max( Math.round( getYOffset() / 20 ) , 1 ); // what frame to animate to
...
function resizeAdjustments() { // fit everything to the screen
	//$('html, body').css('height',(totalFrames*20)+'px'); // increase the height of the document 30 pixels for every frame in the JPEG sequence
